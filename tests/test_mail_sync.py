...
# ---- поддельное IMAP-соединение ---------------------------------------
class FakeImapConnection:
    """Держит состояние «сервера»: словарь folders передаётся снаружи и
    переживает несколько подключений подряд — так же, как один и тот же
    почтовый ящик переживает несколько наших синхронизаций."""
...
class SlowFakeImapConnection(FakeImapConnection):
    """Тот же сервер, но каждый FETCH занимает время — имитация настоящей
    сети, чтобы проверить, что это время не проведено с удержанием
    блокировки базы (см. «конкурентность» ниже)."""
