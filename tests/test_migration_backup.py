...
assert applied == {"006", "008", "009", "011", "012", "013", "015", "016", "019", "020"}, \
    "008/009/011/012/013/015/016/019/020 без down() не откатывались — должны остаться применёнными " \
    "(015/016/019/020 в этом смысле не отличаются от 012: все ALTER'ят или пристраиваются к " \
    "таблице, чей down() уже откатился раньше — 014 для 015/016/019/020, 007 для 012 — " \
    "числящаяся-применённой запись переживает откат основания одинаково во всех случаях; " \
    "013 без down() по той же причине, что и остальные — ALTER на bot_leads плюс новые " \
    "таблицы funnel/funnel_stage, откатывать нечего чисто)"
conn.close()
